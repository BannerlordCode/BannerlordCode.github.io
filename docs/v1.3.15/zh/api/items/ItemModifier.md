<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `ItemModifier / 物品修改?`
- [← 本领域 / 返回 items](./)
- [↑ API 目录](../)
- [🏠 首页 v1.3.15](../../)
...
先把 `ItemModifier` 当作这个子系统的入口或数据节点来理解：先看属性代表什么状态，再看方法允许你做什么。

**Namespace**: TaleWorlds.Core  
**File**: 

`bannerlord-1.3.15/TaleWorlds.Core/ItemModifier.cs

`  
**Purpose**: 物品质量修改?用于调整物品的属性?如伤害、护甲等)

## 关键属?/ Key Properties

\| 属?\| 类型 \| 描述 \|
\|------\|------\|------\|
\| Name \| TextObject \| 修改器名?\|
\| Damage \| int \| 伤害调整?\|
\| Speed \| int \| 速度调整?\|
\| MissileSpeed \| int \| 弹丸速度调整?\|
\| Armor \| int \| 护甲调整?\|
\| HitPoints \| short \| 生命值调整?\|
