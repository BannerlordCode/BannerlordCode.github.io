...
**用途 / Purpose:** 检查item roster has element在当前对象中是否成立。
...
**用途 / Purpose:** 调用 SlaughterItem 对应的操作。
...
**用途 / Purpose:** 调用 DonateItem 对应的操作。
...
**用途 / Purpose:** 调用 TransferOne 对应的操作。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的item from side。
...
**用途 / Purpose:** 调用 TotalAmountChangeDelegate 对应的操作。
...
**用途 / Purpose:** 调用 ProcessResultListDelegate 对应的操作。
...
**用途 / Purpose:** 调用 RecordTransaction 对应的操作。
