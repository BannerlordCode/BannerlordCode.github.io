...
**用途 / Purpose:** 调用 ReturnCachedInventoryTableauScene 对应的操作。
...
**用途 / Purpose:** 调用 ReturnCachedMapConversationTableauScene 对应的操作。
...
**用途 / Purpose:** 将thumbnail cache注册到当前系统，以便后续监听或分发。
...
**用途 / Purpose:** 从当前系统中注销thumbnail cache。
...
**用途 / Purpose:** 调用 ReleaseSandboxValues 对应的操作。
...
**用途 / Purpose:** 调用 DestroyTexture 对应的操作。
...
**用途 / Purpose:** 调用 ForceClearAllCache 对应的操作。
...
**用途 / Purpose:** 清空当前对象中的unused cache。
