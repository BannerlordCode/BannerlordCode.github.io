...
`CapturedByBountyHuntersIssueBehavior` lives in `TaleWorlds.CampaignSystem.Issues` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
// Obtain an instance of CapturedByBountyHuntersIssueBehavior from the subsystem API first
CapturedByBountyHuntersIssueBehavior capturedByBountyHuntersIssueBehavior = ...;
var result = capturedByBountyHuntersIssueBehavior.GetAlternativeSolutionSkill(hero);
```
