...
**用途 / Purpose:** 抛出或触发与event for all peers相关的事件/异常。
...
**用途 / Purpose:** 抛出或触发与event for all peers on team相关的事件/异常。
...
**用途 / Purpose:** 在每一帧或每个更新周期内推进all peer perks的状态。
...
**用途 / Purpose:** 抛出或触发与event for all peers command相关的事件/异常。
...
**用途 / Purpose:** 在每一帧或每个更新周期内推进all peer perks command的状态。
...
**用途 / Purpose:** 读取并返回当前对象中 team gold rewards on death 的结果。
