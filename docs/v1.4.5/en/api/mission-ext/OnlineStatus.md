<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `OnlineStatus`
...
`OnlineStatus` lives in `TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.Friends`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
OnlineStatus example = OnlineStatus.None;
```
