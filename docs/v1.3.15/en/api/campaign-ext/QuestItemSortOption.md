...
`QuestItemSortOption` lives in `TaleWorlds.CampaignSystem.ViewModelCollection.Quests`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
QuestItemSortOption example = QuestItemSortOption.DateStarted;
```
