...
description: "MissionGauntletStoryModeCheatView 的自动生成类参考。"
---
...
`MissionGauntletStoryModeCheatView` 表示一个视图层对象，通常负责把游戏状态投影到屏幕、场景或可交互界面。

## 心智模型

把 `MissionGauntletStoryModeCheatView` 当作一个 View 型扩展点来理解：先确认谁创建它、谁持有它、谁调用它，再决定是继承、组合还是只读使用。
...
```csharp
// 先通过子系统 API 拿到 MissionGauntletStoryModeCheatView 实例
...
**用途 / Purpose:** 处理与 「finalize screen」 相关的逻辑。
