...
**File:** `TaleWorlds.CampaignSystem/CharacterCreationContent/CharacterCreationOptionsStage.cs`

## Overview

`CharacterCreationOptionsStage` lives in `TaleWorlds.CampaignSystem.CharacterCreationContent` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
```csharp
var value = new CharacterCreationOptionsStage();
```
