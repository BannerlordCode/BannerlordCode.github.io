...
**用途 / Purpose:** 获取 `base formation frame` 的当前值。
...
**用途 / Purpose:** 设置 `scripted target entity` 的值或状态。
...
**用途 / Purpose:** 获取 `ground material for collision effect` 的当前值。
...
**用途 / Purpose:** 更新 `direction change tendency` 的状态或数据。
...
**用途 / Purpose:** 设置 `is physics force closed` 的值或状态。
...
**用途 / Purpose:** 判断当前对象是否可以执行 `perform brace`。
...
**用途 / Purpose:** 获取 `sound parameter for armor type` 的当前值。
