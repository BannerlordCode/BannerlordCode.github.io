...
**用途 / Purpose:** 检查for match end在当前对象中是否成立。
...
**用途 / Purpose:** 清空当前对象中的peer counts。
...
**用途 / Purpose:** 调用 ShouldSpawnVisualsForServer 对应的操作。
...
**用途 / Purpose:** 响应 agent visual spawning 事件，执行对应的处理逻辑。
...
**用途 / Purpose:** 调用 ChangeCurrentGoldForPeer 对应的操作。
...
**用途 / Purpose:** 检查if player can despawn在当前对象中是否成立。
