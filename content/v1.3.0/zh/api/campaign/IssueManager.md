...
**用途 / Purpose:** 启动issue quest流程或状态机。
...
**用途 / Purpose:** 调用 ChangeIssueOwner 对应的操作。
...
**用途 / Purpose:** 检查for issues在当前对象中是否成立。
...
**用途 / Purpose:** 检查issue for menu locations在当前对象中是否成立。
...
**用途 / Purpose:** 调用 ToggleAllIssueTracks 对应的操作。
...
**用途 / Purpose:** 调用 FillIssueCountsPerSettlement 对应的操作。
...
**用途 / Purpose:** 调用 FillIssueCountsPerClan 对应的操作。
