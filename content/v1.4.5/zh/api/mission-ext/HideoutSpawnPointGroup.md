...
description: "HideoutSpawnPointGroup 的自动生成类参考。"
---
...
`HideoutSpawnPointGroup` 位于 `TaleWorlds.MountAndBlade`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 spawn point frames 的结果。

```csharp
// 先通过子系统 API 拿到 HideoutSpawnPointGroup 实例
...
**用途 / Purpose:** **用途 / Purpose:** 从当前容器或状态中移除 with all children。
