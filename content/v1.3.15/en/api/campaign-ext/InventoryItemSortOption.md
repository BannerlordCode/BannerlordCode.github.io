...
`InventoryItemSortOption` lives in `TaleWorlds.CampaignSystem.ViewModelCollection.Inventory`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
InventoryItemSortOption example = InventoryItemSortOption.Type;
```
