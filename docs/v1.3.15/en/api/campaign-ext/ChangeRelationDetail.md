...
`ChangeRelationDetail` lives in `TaleWorlds.CampaignSystem.Actions`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
ChangeRelationDetail example = ChangeRelationDetail.Default;
```
