...
**用途 / Purpose:** 调用 NarrowDownCandidates 对应的操作。
...
**用途 / Purpose:** 根据当前状态判定supporters的结果。
...
**用途 / Purpose:** 调用 ShouldBeCancelled 对应的操作。
...
**用途 / Purpose:** 根据当前状态判定support option的结果。
...
**用途 / Purpose:** 调用 SortDecisionOutcomes 对应的操作。
...
**用途 / Purpose:** 计算relationship effect with sponsor的当前值或结果。
