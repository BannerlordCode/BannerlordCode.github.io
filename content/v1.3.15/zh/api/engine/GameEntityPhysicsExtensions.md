...
**用途 / Purpose:** 将 capsule as body 添加到当前容器或状态中。
...
**用途 / Purpose:** 调用 PushCapsuleShapeToEntityBody 对应的操作。
...
**用途 / Purpose:** 调用 PopCapsuleShapeFromEntityBody 对应的操作。
...
**用途 / Purpose:** 读取并返回当前对象中 physics triangle count 的结果。
...
**用途 / Purpose:** 构建一个新的 variable rate physics 实体并返回给调用方。
...
**用途 / Purpose:** 为 physics state only variable 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 调用 ReplacePhysicsBodyWithQuadPhysicsBody 对应的操作。
...
**用途 / Purpose:** 调用 DisableDynamicBodySimulation 对应的操作。
...
**用途 / Purpose:** 调用 DisableDynamicBodySimulationMT 对应的操作。
...
**用途 / Purpose:** 把dynamic body to ray cast转换为另一种表示或类型。
...
**用途 / Purpose:** 调用 EnableDynamicBody 对应的操作。
...
**用途 / Purpose:** 调用 DisableGravity 对应的操作。
...
**用途 / Purpose:** 调用 ComputeVelocityDeltaFromImpulse 对应的操作。
