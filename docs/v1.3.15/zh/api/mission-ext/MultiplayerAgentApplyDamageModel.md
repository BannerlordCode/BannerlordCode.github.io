<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `MultiplayerAgentApplyDamageModel`
- [← 本领域 / 返回 mission-ext](./)
- [↑ API 目录](../)
...
`MultiplayerAgentApplyDamageModel` 是一个游戏 Model——规则/覆盖点。modder 继承它并经 `Game.Current.ReplaceModel<MultiplayerAgentApplyDamageModel>(new MyMultiplayerAgentApplyDamageModel())` 注册，以改变其计算逻辑。
...
```csharp
// MultiplayerAgentApplyDamageModel (Model) 的典型用法
