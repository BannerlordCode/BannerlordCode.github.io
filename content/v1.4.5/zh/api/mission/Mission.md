...
**用途 / Purpose:** 处理与 「compute troop body properties delegate」 相关的逻辑。
...
**用途 / Purpose:** 激活「mission object」对应的资源、状态或功能。
...
**用途 / Purpose:** 停用「mission object」对应的资源、状态或功能。
...
**用途 / Purpose:** 在每一帧或每个更新周期内推进「agents and teams async」的状态。
...
**用途 / Purpose:** 为即将执行的「missile weapon for drop」操作完成前置准备工作。
...
**用途 / Purpose:** 清空当前对象中的「agent actions」。
...
**用途 / Purpose:** 清空当前对象中的「unreferenced resources」。
...
**用途 / Purpose:** 处理与 「force disable occlusion」 相关的逻辑。
...
**用途 / Purpose:** 在每一帧或每个更新周期内推进「agents and teams imp」的状态。
...
**用途 / Purpose:** 把若干「enemy team」连接成一个整体。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的「best defending position」。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的「position with biggest slope towards direction in square」。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的「agent with index」。
...
**用途 / Purpose:** 处理与 「compute spawn path deployment offset」 相关的逻辑。
...
**用途 / Purpose:** 显示「in mission loading screen」对应的界面或元素。
...
**用途 / Purpose:** 执行与 「missile collision reaction」 相关的响应逻辑。
...
**用途 / Purpose:** 处理与 「take control of agent」 相关的逻辑。
...
**用途 / Purpose:** 检查「if battle in retreat」在当前对象中是否成立。
...
**用途 / Purpose:** 触发「on item pick up event」对应的逻辑或事件。
