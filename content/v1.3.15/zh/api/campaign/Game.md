...
先把 `Game` 当作这个子系统的入口或数据节点来理解：先看属性代表什么状态，再看方法允许你做什么。

**Namespace**: TaleWorlds.Core  
**File**: 

`bannerlord-1.3.15/TaleWorlds.Core/Game.cs
...
`AddGameHandler&lt;T&gt;() where T : GameHandler, new()
...
` \| Type of game (Campaign, CustomBattle, etc.) \|
\| 
...
` \| Object manager for game objects \|
\| 
...
`void Save(MetaData metaData, string saveName, ISaveDriver driver, Action onSaveCompleted)
...
`static void RegisterTypes(GameType gameType, MBObjectManager objectManager, GameManagerBase gameManager)
...
`void SetBasicModels(IEnumerable models)
...
`T AddGameModelsManager(IEnumerable inputComponents) where T : GameModelsManager
...
`T AddGameHandler() where T : GameHandler, new()
...
`T GetGameHandler() where T : GameHandler
...
`void RemoveGameHandler() where T : GameHandler
...
`void SetDefaultEquipments(IReadOnlyDictionary defaultEquipments)
...
` \| Fired when item is deserialized \|
...
` attribute (ID 5000) for save system integration. Key save-related methods:
- 
