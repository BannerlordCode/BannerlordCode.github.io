...
`ValueCategorization` lives in `TaleWorlds.Core.ViewModelCollection.Information.RundownTooltip`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
ValueCategorization example = ValueCategorization.None;
```
