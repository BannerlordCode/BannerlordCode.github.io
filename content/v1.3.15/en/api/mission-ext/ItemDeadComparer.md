...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `ItemDeadComparer`
...
**Type:** `public class ItemDeadComparer : SPScoreboardSortControllerVM.ScoreboardUnitItemComparerBase`
**Base:** `SPScoreboardSortControllerVM.ScoreboardUnitItemComparerBase`
**Area:** mission-ext

## Overview

`ItemDeadComparer` lives in `TaleWorlds.MountAndBlade.ViewModelCollection.Scoreboard`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a ItemDeadComparer instance from game state, then call one of its public methods
var value = new ItemDeadComparer();
