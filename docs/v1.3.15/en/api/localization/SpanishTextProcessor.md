<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `SpanishTextProcessor`
...
`SpanishTextProcessor` lives in `TaleWorlds.Localization.TextProcessor.LanguageProcessors`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a SpanishTextProcessor instance from game state, then call one of its public methods
