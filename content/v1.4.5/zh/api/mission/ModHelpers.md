...
**用途 / Purpose:** 判断scene folder already exist这一条件在当前对象上是否成立。
...
**用途 / Purpose:** 从当前实例读取scene name of directory相关数据。
...
**用途 / Purpose:** 将buffer to temp file写入目标位置。
...
**用途 / Purpose:** 调用 ExtractZipToTempDirectory 对应的操作。
...
**用途 / Purpose:** 调用 DownloadToTempFile 对应的操作。
