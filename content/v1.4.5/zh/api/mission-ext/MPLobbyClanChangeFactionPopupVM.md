...
description: "MPLobbyClanChangeFactionPopupVM 的自动生成类参考。"
---
...
`MPLobbyClanChangeFactionPopupVM` 位于 `TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.Clan`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。
...
```csharp
// 先通过子系统 API 拿到 MPLobbyClanChangeFactionPopupVM 实例
...
**用途 / Purpose:** **用途 / Purpose:** 执行 change faction 对应的操作或工作流。
