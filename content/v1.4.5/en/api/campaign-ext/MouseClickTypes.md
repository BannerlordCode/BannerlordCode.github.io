---
title: "MouseClickTypes"
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `MouseClickTypes`
- [← Area / Back to campaign-ext](./)
- [↑ API Index](../)
- [🏠 Home v1.4.5](../../)
- [⭐ Version Architecture](../../architecture/)
<!-- END BREADCRUMB -->
# MouseClickTypes

**Namespace:** StoryMode.ViewModelCollection.Missions
**Module:** StoryMode.ViewModelCollection
**Type:** `public enum MouseClickTypes`
**Area:** campaign-ext

## Overview

`MouseClickTypes` lives in `StoryMode.ViewModelCollection.Missions`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.

## Mental Model

Start from namespace `StoryMode.ViewModelCollection.Missions` to place it in the stack, then inspect its public methods: if it mainly exposes Get/Set members, it is likely a state object; if it centers on Create/Apply/Execute verbs, it behaves more like a service or workflow entry point.

## Enum Values

| Name |
|------|
| `Left` |
| `Middle` |

## Usage Example

```csharp
MouseClickTypes example = MouseClickTypes.Left;
```

## See Also

- [Complete Class Catalog](../catalog)
- [Area catalog](../catalog-campaign)
