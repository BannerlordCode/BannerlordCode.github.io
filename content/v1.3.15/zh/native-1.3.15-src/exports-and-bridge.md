...
先把 `导出与托管桥接` 当作这个子系统的入口或数据节点来理解：先看属性代表什么状态，再看方法允许你做什么。
...
`WotsMainSDLL` 是托管启动代码最直接可见的入口。在 [LibraryApplicationInterface 文档](../../native/LibraryApplicationInterface/) 中，`MBDotNet.WotsMainDotNet` 通过 `DllImport("TaleWorlds.Native.dll", EntryPoint = "WotsMainSDLL")` 绑定到它。
...
托管侧的 `[EngineClass("ftdnNative_array")]` 等特性在 [EngineClass 与 EngineMethod](../../native/EngineMethod/) 中有说明。反编译侧的字符串和 vftable 写入可以用来追踪原生对象创建、引用计数和跨边界返回值。
...
该类型与 `rglScript_component`、`rglManaged_script_component` 同属场景脚本组件分析入口。详见 [场景系统](../scene/)。

## 与托管 Native 文档的对应关系 / Mapping To Managed Native Reference

| 托管文档 | 原生侧锚点 |
|----------|------------|
| [NativeObject](../../native/NativeObject/) | `IManaged` 引用计数、`EngineClass` 类型注册、`ftdn*` 原生对象 |
| [INativeArray](../../native/INativeArray/) | `ftdnNative_array` 字符串、vftable 写入、`create/get_data_*` 系列方法 |
| [INativeString](../../native/INativeString/) | `ftdnNative_string` 字符串和字符串 helper |
| [INativeObjectArray](../../native/INativeObjectArray/) | `ftdnNative_object_array` 字符串和对象数组方法 |
| [LibraryApplicationInterface](../../native/LibraryApplicationInterface/) | `pass_managed_library_callback_method_pointers`、`ScriptingInterfaceObjects.SetFunctionPointer` |
| [EngineMethod](../../native/EngineMethod/) | `[EngineMethod]` 名称与原生注册表/回调表之间的映射 |
