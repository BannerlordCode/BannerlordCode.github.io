...
`NotablesCampaignBehavior` 位于 `TaleWorlds.CampaignSystem.CampaignBehaviors`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。
...
**用途 / Purpose:** **用途 / Purpose:** 为 initial relations between notables and lords 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** **用途 / Purpose:** 在 new game created partial follow up 事件触发时调用此回调。
