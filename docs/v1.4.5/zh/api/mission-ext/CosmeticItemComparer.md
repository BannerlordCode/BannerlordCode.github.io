<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `CosmeticItemComparer`
- [← 本领域 / 返回 mission-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.4.5](../../)
- [⭐ 版本架构](../../architecture/)
...
**类型:** `public abstract class CosmeticItemComparer : IComparer<MPArmoryCosmeticItemBaseVM>`
**Base:** `IComparer<MPArmoryCosmeticItemBaseVM>`
**领域:** mission-ext

## 概述

`CosmeticItemComparer` 位于 `TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.Armory`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
