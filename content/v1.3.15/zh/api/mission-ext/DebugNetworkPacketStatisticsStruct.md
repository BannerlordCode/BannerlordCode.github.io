...
---
<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `DebugNetworkPacketStatisticsStruct`
- [← 本领域 / 返回 mission-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.3.15](../../)
...
**类型:** `public struct DebugNetworkPacketStatisticsStruct`
**领域:** mission-ext

## 概述

`DebugNetworkPacketStatisticsStruct` 位于 `TaleWorlds.MountAndBlade`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
DebugNetworkPacketStatisticsStruct.ClearAllPeers();
```
