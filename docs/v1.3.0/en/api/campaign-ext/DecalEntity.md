<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `DecalEntity`
...
`DecalEntity` lives in `SandBox.View.Map`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a DecalEntity instance from game state, then call one of its public methods
var value = new DecalEntity();
