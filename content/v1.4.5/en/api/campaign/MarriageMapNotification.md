...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `MarriageMapNotification`
...
**File:** `Bannerlord.Source/bin/TaleWorlds.CampaignSystem/TaleWorlds.CampaignSystem.MapNotificationTypes/MarriageMapNotification.cs`

## Overview

`MarriageMapNotification` lives in `TaleWorlds.CampaignSystem.MapNotificationTypes` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
```csharp
var example = new MarriageMapNotification();
```
