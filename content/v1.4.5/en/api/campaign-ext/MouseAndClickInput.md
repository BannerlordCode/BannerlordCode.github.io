...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `MouseAndClickInput`
...
`MouseAndClickInput` lives in `StoryMode.ViewModelCollection.Missions`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a MouseAndClickInput instance from game state, then call one of its public methods
var value = new MouseAndClickInput();
