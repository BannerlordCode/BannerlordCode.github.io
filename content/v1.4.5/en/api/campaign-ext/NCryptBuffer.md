...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `NCryptBuffer`
...
**Module:** Jose.native
**Type:** `public class NCryptBuffer`
**Area:** campaign-ext

## Overview

`NCryptBuffer` lives in `Jose.native`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a NCryptBuffer instance from game state, then call one of its public methods
var value = new NCryptBuffer();
value.Dispose();
```
