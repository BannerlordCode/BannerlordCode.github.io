...
`BannerBearerLogic` 跟踪队形旗帜——哪个 agent 扛旗、旗是否在地上，并能查找某队形的活动旗帜（`GetActiveBanner`、`HasBannerOnGround`）。用于在攻城/战斗任务中查询或操作旗手状态。
...
foreach (Formation f in Mission.Current.PlayerTeam.Formations)
{
    ...
    InformationManager.DisplayMessage(new InformationMessage(
        $"{f.CountOfUnits} 人 — 旗帜 {(onAgent ? "扛着" : onGround ? "在地上" : "无")}"));
}
```
