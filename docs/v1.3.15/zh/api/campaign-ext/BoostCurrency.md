...
**类型:** `public enum BoostCurrency`
**领域:** campaign-ext

## 概述

`BoostCurrency` 位于 `TaleWorlds.CampaignSystem.ViewModelCollection.ArmyManagement`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
```csharp
BoostCurrency example = BoostCurrency.Gold;
```
