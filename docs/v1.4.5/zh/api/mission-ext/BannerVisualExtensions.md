...
**类型:** `public static class BannerVisualExtensions`
**领域:** mission-ext

## 概述

`BannerVisualExtensions` 位于 `TaleWorlds.MountAndBlade.View`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
**用途 / Purpose:** 获取 `tableau texture small for banner editor` 的当前值。
...
**用途 / Purpose:** 获取 `tableau texture large for banner editor` 的当前值。
