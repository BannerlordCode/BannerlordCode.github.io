---
title: "MultiThreadedTestFunctions"
description: "Auto-generated class reference for MultiThreadedTestFunctions."
---
# MultiThreadedTestFunctions

**Namespace:** TaleWorlds.MountAndBlade.View.Screens.Scripts
**Module:** TaleWorlds.MountAndBlade
**Type:** `public static class MultiThreadedTestFunctions`
**Base:** none
**File:** `TaleWorlds.MountAndBlade.View/TaleWorlds/MountAndBlade/View/Screens/Scripts/MultiThreadedStressTestsScreen.cs`

## Overview

`MultiThreadedTestFunctions` lives in `TaleWorlds.MountAndBlade.View.Screens.Scripts` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.

## Mental Model

Start from namespace `TaleWorlds.MountAndBlade.View.Screens.Scripts` to place it in the stack, then inspect its public methods: if it mainly exposes Get/Set members, it is likely a state object; if it centers on Create/Apply/Execute verbs, it behaves more like a service or workflow entry point.

## Key Methods

### MeshMerger
`public static void MeshMerger(InputLayout layout)`

**Purpose:** Executes the MeshMerger logic.

```csharp
// Static call; no instance required
MultiThreadedTestFunctions.MeshMerger(layout);
```

### SceneHandler
`public static void SceneHandler(SceneView view)`

**Purpose:** Executes the SceneHandler logic.

```csharp
// Static call; no instance required
MultiThreadedTestFunctions.SceneHandler(view);
```

## Usage Example

```csharp
MultiThreadedTestFunctions.MeshMerger(layout);
```

## See Also

- [Area Index](../)