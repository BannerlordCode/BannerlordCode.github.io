...
`StartAllianceDecisionOutcome` lives in `TaleWorlds.CampaignSystem.Election`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
StartAllianceDecisionOutcome.GetProposerClanForPlayerKingdom(target);
```
