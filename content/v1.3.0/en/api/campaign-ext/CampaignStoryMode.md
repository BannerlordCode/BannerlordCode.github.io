---
title: "CampaignStoryMode"
description: "Auto-generated class reference for CampaignStoryMode."
---
# CampaignStoryMode

**Namespace:** StoryMode
**Module:** StoryMode
**Type:** `public class CampaignStoryMode : Campaign`
**Base:** `Campaign`
**File:** `StoryMode/CampaignStoryMode.cs`

## Overview

`CampaignStoryMode` lives in `StoryMode` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.

## Mental Model

Start from namespace `StoryMode` to place it in the stack, then inspect its public methods: if it mainly exposes Get/Set members, it is likely a state object; if it centers on Create/Apply/Execute verbs, it behaves more like a service or workflow entry point.

## Key Properties

| Name | Signature |
|------|-----------|
| `StoryMode` | `public StoryModeManager StoryMode { get; }` |

## Usage Example

```csharp
// Obtain an instance from the relevant subsystem API
CampaignStoryMode instance = ...;
```

## See Also

- [Area Index](../)