...
**类型:** `public class MultiThreadedStressTestsScreen : ScreenBase`
**Base:** `ScreenBase`
**领域:** mission-ext

## 概述

`MultiThreadedStressTestsScreen` 位于 `TaleWorlds.MountAndBlade.View.Screens.Scripts`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
