...
- 上游 / 相关：`[Scene](../Scene)`（唯一被 mod 持有的场景托管类，封装 `IScene` 边界）、`[Mission](../../mission/Mission)`（`Mission.Current.Scene` 提供战斗/任务场景）、`[Campaign](../../campaign/Campaign)`（`IMapScene` / `MapSceneWrapper` 提供地图场景）、`NativeObject`（`Scene` 的基类，原生句柄模型）、`[EngineApplicationInterface](../EngineApplicationInterface)`（引擎侧接口总入口）、`[GameEntity](../GameEntity)`（`Scene` 中承载的实体）。
