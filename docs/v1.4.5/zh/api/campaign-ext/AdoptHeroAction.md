...
先把 `AdoptHeroAction` 当作这个子系统的入口或数据节点来理解：先看属性代表什么状态，再看方法允许你做什么。
...
AdoptHeroAction 是一组静态方法，用于在战役中以特定原因触发"AdoptHero"。modder通过调用其 `Apply*` 方法改变游戏状态（每种原因一个重载）。
