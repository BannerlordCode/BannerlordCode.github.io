...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `ItemRemainingComparer`
...
**Type:** `public class ItemRemainingComparer : SPScoreboardSortControllerVM.ScoreboardUnitItemComparerBase`
**Base:** `SPScoreboardSortControllerVM.ScoreboardUnitItemComparerBase`
**Area:** mission-ext

## Overview

`ItemRemainingComparer` lives in `TaleWorlds.MountAndBlade.ViewModelCollection.Scoreboard`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a ItemRemainingComparer instance from game state, then call one of its public methods
