...
**Type:** `public class ItemQuantityComparer : SPInventorySortControllerVM.ItemComparer`
**Base:** `SPInventorySortControllerVM.ItemComparer`
**Area:** campaign-ext

## Overview

`ItemQuantityComparer` lives in `TaleWorlds.CampaignSystem.ViewModelCollection.Inventory`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a ItemQuantityComparer instance from game state, then call one of its public methods
