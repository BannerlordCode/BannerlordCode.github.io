---
title: "Doc Contract — Handwritten-Only Policy and Acceptance (v1.4.5)"
description: "Hard rules for the BannerlordCode v1.4.5 docs: H1–H9 sacred constraints, deep/family page MUST fields, auto-reject boilerplate, banned body generators, R1 scope and tool allowlist. The highest writing and review constraint for this version."
---
# Doc Contract — Handwritten-Only Policy and Acceptance (v1.4.5)

> **This page is the highest writing and review constraint for the v1.4.5 docs.** Violations are failures. Any agent, script, or contributor must read this contract before writing class-doc body prose.
>
...
Chinese is primary where a matching page exists. This English page is the authoritative documentation contract for **v1.4.5**.
...
## 5. R1 scope and noise (v1.4.5 authority)

- **Authority:** usage semantics from full `bannerlord-1.4.5` (`Bannerlord.Source/bin/...`). Core API notes vs `1.3.15` where relevant.
...
- **Versions:** one main handbook (this v1.4.5 tree) + notes/S-tier diffs. **No** triple empty shell forests.
...
Each banned script now carries a hard-fail guard (see `tools/RETIRED_BODY_GENERATORS.md`): it exits 1 unless `BANNERLORD_ALLOW_RETIRED_BODY_GEN=1` is set (local archaeology only, forbidden for product builds/CI/commits).
