...
**Base:** `EventBase`
**File:** `Bannerlord.Source/Modules.SandBox/SandBox/SandBox.Missions/CheckpointLoadedMissionEvent.cs`

## Overview

`CheckpointLoadedMissionEvent` lives in `SandBox.Missions` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
