...
**用途 / Purpose:** 读取并返回当前对象中 normalized war progress tooltip 的结果。
...
**用途 / Purpose:** 调用 MergeTextObjectsWithNewline 对应的操作。
...
**用途 / Purpose:** 读取并返回当前对象中 smithing difficulty tooltip 的结果。
...
**用途 / Purpose:** 把to hex color转换为另一种表示或类型。
...
**用途 / Purpose:** 调用 FloatToString 对应的操作。
