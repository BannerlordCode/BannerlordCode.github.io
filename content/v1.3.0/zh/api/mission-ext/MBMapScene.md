...
**用途 / Purpose:** 在每一帧或每个更新周期内推进step sound的状态。
...
**用途 / Purpose:** 在每一帧或每个更新周期内推进ambient sounds的状态。
...
**用途 / Purpose:** 调用 SendMouseKeyEvent 对应的操作。
...
**用途 / Purpose:** 检查terrain sound ids是否满足约束条件，通常返回布尔值。
