...
`TaleWorlds.MountAndBlade/HumanAIComponent.cs
...
` 是挂在人类（NPC）agent 上的 AI 驱动组件。它拥有行为参数、脚本化移动帧（

`HasTimedScriptedFrame

`）、防?可用物体兴趣状态。mod 用它覆盖 AI 行为参数，或?agent 放到脚本化帧（如哨位）?
## 
## 心智模型

先把 `HumanAIComponent` 当作这个子系统的入口或数据节点来理解：先看属性代表什么状态，再看方法允许你做什么。
主要属?
\| Name \| Signature \|
\|------\|-----------\|
\| 
...
`public bool ShouldCatchUpWithFormation { get { return this._shouldCatchUpWithFormation; }
...
`public bool IsDefending { get { return this._objectInterestKind == HumanAIComponent.UsableObjectInterestKind.Defending; }
...
`public bool HasTimedScriptedFrame { get { return this._scriptedFrameTimer &gt; 0f; }
