...
**用途 / Purpose:** 更新 `body rest offset` 的状态或数据。
...
**用途 / Purpose:** 创建一个 `variable rate physics` 实例或对象。
...
**用途 / Purpose:** 设置 `physics state only variable` 的值或状态。
...
**用途 / Purpose:** 处理 `compute velocity delta from impulse` 相关逻辑。
