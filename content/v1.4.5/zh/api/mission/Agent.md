...
**用途 / Purpose:** 调用 IncreaseDamage 对应的操作。
...
**用途 / Purpose:** 将last blow注册到当前系统，以便后续监听或分发。
...
**用途 / Purpose:** 调用 DebugGetHealth 对应的操作。
...
**用途 / Purpose:** 启动ragdoll as corpse流程或状态机。
...
**用途 / Purpose:** 调用 EndRagdollAsCorpse 对应的操作。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的longest direct move to position。
...
**用途 / Purpose:** 读取并返回当前对象中 base formation frame 的结果。
...
**用途 / Purpose:** 在 conversation started 事件触发时调用此回调。
...
**用途 / Purpose:** 为 scripted target entity 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 读取并返回当前对象中 ground material for collision effect 的结果。
...
**用途 / Purpose:** 读取并返回当前对象中 total mass 的结果。
...
**用途 / Purpose:** 调用 DeleteAttachedWeapon 对应的操作。
...
**用途 / Purpose:** 调用 ToggleInvulnerable 对应的操作。
...
**用途 / Purpose:** 检查path to a i target agent passes through navigation face id from direction在当前对象中是否成立。
...
**用途 / Purpose:** 检查equipment for cape cloth simulation state change在当前对象中是否成立。
...
**用途 / Purpose:** 检查to drop flagged item在当前对象中是否成立。
...
**用途 / Purpose:** 检查skill for mounting在当前对象中是否成立。
...
**用途 / Purpose:** 调用 ForceUpdateCachedAndFormationValues 对应的操作。
...
**用途 / Purpose:** 重新计算并更新 direction change tendency 的最新表示。
...
**用途 / Purpose:** 调用 InvalidateTargetAgent 对应的操作。
...
**用途 / Purpose:** 调用 InvalidateAIWeaponSelections 对应的操作。
...
**用途 / Purpose:** 清空当前对象中的target frame。
...
**用途 / Purpose:** 清空当前对象中的hand inverse kinematics。
...
**用途 / Purpose:** 清空当前对象中的attached weapons。
...
**用途 / Purpose:** 尝试获取 attach to formation 的值，通常通过 out 参数返回是否成功。
...
**用途 / Purpose:** 尝试获取 remove all detachment scores 的值，通常通过 out 参数返回是否成功。
...
**用途 / Purpose:** 尝试获取 set formation frame 的值，通常通过 out 参数返回是否成功。
...
**用途 / Purpose:** 调用 EnforceShieldUsage 对应的操作。
...
**用途 / Purpose:** 调用 ObjectHasVacantPosition 对应的操作。
...
**用途 / Purpose:** 调用 InteractingWithAnyGameObject 对应的操作。
...
**用途 / Purpose:** 停止using game object m t流程或状态机。
...
**用途 / Purpose:** 停止using game object流程或状态机。
...
**用途 / Purpose:** 响应 stop using action 事件，执行对应的处理逻辑。
...
**用途 / Purpose:** 响应 start using action 事件，执行对应的处理逻辑。
...
**用途 / Purpose:** 启动switching weapon usage index as client流程或状态机。
...
**用途 / Purpose:** 为即将执行的weapon for drop in equipment slot操作完成前置准备工作。
...
**用途 / Purpose:** 调用 UseGameObject 对应的操作。
...
**用途 / Purpose:** 调用 ComputeAnimationDisplacement 对应的操作。
...
**用途 / Purpose:** 为 is physics force closed 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 调用 LockAgentReplicationTableDataWithCurrentReliableSequenceNo 对应的操作。
...
**用途 / Purpose:** 调用 TeleportToPosition 对应的操作。
...
**用途 / Purpose:** 调用 DisableScriptedMovement 对应的操作。
...
**用途 / Purpose:** 调用 DisableScriptedCombatMovement 对应的操作。
...
**用途 / Purpose:** 调用 ForceAiBehaviorSelection 对应的操作。
...
**用途 / Purpose:** 调用 DisableLookToPointOfInterest 对应的操作。
...
**用途 / Purpose:** 调用 YellAfterDelay 对应的操作。
...
**用途 / Purpose:** 调用 WieldNextWeapon 对应的操作。
...
**用途 / Purpose:** 调用 AttackDirectionToMovementFlag 对应的操作。
...
**用途 / Purpose:** 调用 DefendDirectionToMovementFlag 对应的操作。
...
**用途 / Purpose:** 调用 PlayerAttackDirection 对应的操作。
...
**用途 / Purpose:** 响应 taunt 事件，执行对应的处理逻辑。
...
**用途 / Purpose:** 响应 bark 事件，执行对应的处理逻辑。
...
**用途 / Purpose:** 响应 drop weapon 事件，执行对应的处理逻辑。
...
**用途 / Purpose:** 调用 EquipItemsFromSpawnEquipment 对应的操作。
...
**用途 / Purpose:** 调用 WieldInitialWeapons 对应的操作。
...
**用途 / Purpose:** 调用 ChangeWeaponHitPoints 对应的操作。
...
**用途 / Purpose:** 调用 AttachWeaponToBone 对应的操作。
...
**用途 / Purpose:** 调用 RestoreShieldHitPoints 对应的操作。
...
**用途 / Purpose:** 将blow注册到当前系统，以便后续监听或分发。
...
**用途 / Purpose:** 在每一帧或每个更新周期内推进parallel的状态。
...
**用途 / Purpose:** 调用 EquipWeaponToExtraSlotAndWield 对应的操作。
...
**用途 / Purpose:** 调用 EquipWeaponWithNewEntity 对应的操作。
...
**用途 / Purpose:** 调用 EquipWeaponFromSpawnedItemEntity 对应的操作。
...
**用途 / Purpose:** 调用 WillDropWieldedShield 对应的操作。
...
**用途 / Purpose:** 调用 HadSameTypeOfConsumableOrShieldOnSpawn 对应的操作。
...
**用途 / Purpose:** 检查当前对象是否满足 perform brace 的前置条件。
...
**用途 / Purpose:** 调用 YellingBehaviour 对应的操作。
...
**用途 / Purpose:** 移动ment flag to direction到新的位置或状态。
...
**用途 / Purpose:** 读取并返回当前对象中 sound parameter for armor type 的结果。
