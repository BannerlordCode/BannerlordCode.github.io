...
**命名空间:** SandBox.GauntletUI.Tutorial
**模块:** SandBox.GauntletUI
**类型:** `public class GauntletTutorialSystem : GlobalLayer`
**Base:** `GlobalLayer`
**领域:** campaign-ext

## 概述

`GauntletTutorialSystem` 位于 `SandBox.GauntletUI.Tutorial`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
