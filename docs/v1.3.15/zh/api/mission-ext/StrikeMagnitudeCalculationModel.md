...
`StrikeMagnitudeCalculationModel` 是一个游戏 Model——规则/覆盖点。modder 继承它并经 `Game.Current.ReplaceModel<StrikeMagnitudeCalculationModel>(new MyStrikeMagnitudeCalculationModel())` 注册，以改变其计算逻辑。
...
```csharp
// StrikeMagnitudeCalculationModel (Model) 的典型用法
