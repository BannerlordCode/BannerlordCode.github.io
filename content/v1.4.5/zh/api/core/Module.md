...
**用途 / Purpose:** 调用 CollectSubModules 对应的操作。
...
**用途 / Purpose:** 读取并返回当前对象中 crafted item mesh names 的结果。
...
**用途 / Purpose:** 检查if submodule can be loadable在当前对象中是否成立。
...
**用途 / Purpose:** 清空当前对象中的state options。
...
**用途 / Purpose:** 调用 OverrideInitialStateOption 对应的操作。
...
**用途 / Purpose:** 启动mission for editor aux流程或状态机。
...
**用途 / Purpose:** 调用 DeactiveModule 对应的操作。
