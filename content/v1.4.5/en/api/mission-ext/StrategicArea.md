...
**Purpose:** Reads and returns the `ground position` value held by the current object.
...
**Purpose:** Determines the result of `associated destructible components` based on the current state.
...
**Purpose:** Invoked when the `parent game entity visibility changed` event is raised.
...
**Purpose:** Prepares the resources, state, or bindings required by `autogenerated`.
...
**Purpose:** Determines whether the current object is in the `agent eligible` state or condition.
...
**Purpose:** Determines whether the current object is in the `usable by` state or condition.
