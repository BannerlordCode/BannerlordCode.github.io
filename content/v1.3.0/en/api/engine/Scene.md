...
// Obtain an instance of Scene from the subsystem API first
Scene scene = ...;
scene.GetBulkWaterLevelAtVolumes(waterHeightQueryArray, globalFrame);
```
...
// Obtain an instance of Scene from the subsystem API first
Scene scene = ...;
var result = scene.GenerateContactsWithCapsule(capsule, exclude_flags, intersectionsOutput, gameEntities, entityPointers);
```
...
// Obtain an instance of Scene from the subsystem API first
Scene scene = ...;
var result = scene.RayCastForRamming(sourcePoint, targetPoint, ignoredEntity, collisionDistance, intersectionPoint, collidedEntity, bodyFlags.CommonFocusRayCastExcludeFlags, bodyFlags.None);
```
