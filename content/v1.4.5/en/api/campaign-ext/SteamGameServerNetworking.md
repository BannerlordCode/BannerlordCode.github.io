...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `SteamGameServerNetworking`
...
**Module:** Steamworks
**Type:** `public static class SteamGameServerNetworking`
**Base:** none
**File:** `Bannerlord.Source/bin/Steamworks.NET/Steamworks/SteamGameServerNetworking.cs`

## Overview

`SteamGameServerNetworking` lives in `Steamworks` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
```csharp
SteamGameServerNetworking.SendP2PPacket(steamIDRemote, 0, 0, eP2PSendType, 0);
```
