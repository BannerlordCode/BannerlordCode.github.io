...
**类型:** `public class PolicyDecisionOutcome : DecisionOutcome`
**Base:** `DecisionOutcome`
**领域:** campaign-ext

## 概述

`PolicyDecisionOutcome` 位于 `TaleWorlds.CampaignSystem.Election`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
```csharp
// 先从游戏状态中拿到一个 PolicyDecisionOutcome 实例，再调用它的公开方法
