<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `RuralNotableInnAndOutIssueQuest`
- [← 本领域 / 返回 campaign-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.3.0](../../)
- [⭐ 版本架构](../../architecture/)
...
**命名空间:** SandBox.Issues
**模块:** SandBox.Issues
**类型:** `public class RuralNotableInnAndOutIssueQuest : QuestBase`
**Base:** `QuestBase`
**领域:** campaign-ext

## 概述

`RuralNotableInnAndOutIssueQuest` 位于 `SandBox.Issues`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
```csharp
// 先从游戏状态中拿到一个 RuralNotableInnAndOutIssueQuest 实例，再调用它的公开方法
