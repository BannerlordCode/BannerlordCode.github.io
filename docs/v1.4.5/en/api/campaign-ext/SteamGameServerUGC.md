<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `SteamGameServerUGC`
...
**Module:** Steamworks
**Type:** `public static class SteamGameServerUGC`
**Base:** none
**File:** `Bannerlord.Source/bin/Steamworks.NET/Steamworks/SteamGameServerUGC.cs`

## Overview

`SteamGameServerUGC` lives in `Steamworks` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
```csharp
SteamGameServerUGC.CreateQueryUserUGCRequest(unAccountID, eListType, eMatchingUGCType, eSortOrder, nCreatorAppID, nConsumerAppID, 0);
```
