---
title: "GuardsCampaignBehavior"
description: "GuardsCampaignBehavior 的自动生成类参考。"
---
# GuardsCampaignBehavior

**Namespace:** SandBox.CampaignBehaviors
**Module:** SandBox.CampaignBehaviors
**Type:** `public class GuardsCampaignBehavior : CampaignBehaviorBase`
**Base:** `CampaignBehaviorBase`
**File:** `Modules.SandBox/SandBox/SandBox.CampaignBehaviors/GuardsCampaignBehavior.cs`

## 概述

`GuardsCampaignBehavior` 位于 `SandBox.CampaignBehaviors`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。

## 心智模型

先从命名空间 `SandBox.CampaignBehaviors` 判断它属于哪层系统，再看公开方法：如果以 Get/Set 为主，它多半是状态对象；如果以 Create/Apply/Execute 为主，它更像服务或流程入口。

## 主要方法

### RegisterEvents
`public override void RegisterEvents()`

**用途 / Purpose:** 将「events」注册到当前系统，以便后续监听或分发。

```csharp
// 先通过子系统 API 拿到 GuardsCampaignBehavior 实例
GuardsCampaignBehavior guardsCampaignBehavior = ...;
guardsCampaignBehavior.RegisterEvents();
```

### SyncData
`public override void SyncData(IDataStore dataStore)`

**用途 / Purpose:** 将「data」同步到相关上下文或系统中。

```csharp
// 先通过子系统 API 拿到 GuardsCampaignBehavior 实例
GuardsCampaignBehavior guardsCampaignBehavior = ...;
guardsCampaignBehavior.SyncData(dataStore);
```

### OnSessionLaunched
`public void OnSessionLaunched(CampaignGameStarter campaignGameStarter)`

**用途 / Purpose:** 在 「session launched」 事件触发时调用此回调。

```csharp
// 先通过子系统 API 拿到 GuardsCampaignBehavior 实例
GuardsCampaignBehavior guardsCampaignBehavior = ...;
guardsCampaignBehavior.OnSessionLaunched(campaignGameStarter);
```

## 使用示例

```csharp
// 通常从对应子系统 API 获取实例后调用
GuardsCampaignBehavior guardsCampaignBehavior = ...;
guardsCampaignBehavior.RegisterEvents();
```

## 参见

- [本区域目录](../)