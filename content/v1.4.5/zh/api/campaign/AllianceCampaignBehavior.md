...
**用途 / Purpose:** 当 `alliance offered to player` 事件触发时调用此方法。

### OnAllianceOfferedToPlayerKingdom
`public void OnAllianceOfferedToPlayerKingdom(Kingdom offeringKingdom)`

**用途 / Purpose:** 当 `alliance offered to player kingdom` 事件触发时调用此方法。
...
**用途 / Purpose:** 当 `call to war agreement proposed to player` 事件触发时调用此方法。
...
**用途 / Purpose:** 当 `call to war agreement proposed to player kingdom` 事件触发时调用此方法。
...
**用途 / Purpose:** 当 `call to war agreement proposed by player` 事件触发时调用此方法。

### GetAllianceEndDate
`public CampaignTime GetAllianceEndDate(Kingdom kingdom1, Kingdom kingdom2)`

**用途 / Purpose:** 获取 `alliance end date` 的当前值。
...
**用途 / Purpose:** 处理 `deny call to war agreement` 相关逻辑。
