...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `DelayedAction`
...
`DelayedAction` lives in `StoryMode.Missions`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a DelayedAction instance from game state, then call one of its public methods
var value = new DelayedAction();
value.OnCreated();
```
