...
**Base:** `LogEntry`
**File:** `TaleWorlds.CampaignSystem/LogEntries/KingdomDestroyedLogEntry.cs`

## Overview

`KingdomDestroyedLogEntry` lives in `TaleWorlds.CampaignSystem.LogEntries` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
