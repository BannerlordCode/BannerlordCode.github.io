---
title: "ILobbyMemberDataUpdateListener"
---
<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `ILobbyMemberDataUpdateListener`
- [← 本领域 / 返回 campaign-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.4.5](../../)
- [⭐ 版本架构](../../architecture/)
...
`ILobbyMemberDataUpdateListener` 位于 `Galaxy.Api`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。
...
**用途 / Purpose:** 处理 `swig delegate i lobby member data update listener_0` 相关逻辑。
...
**用途 / Purpose:** 处理 `swig delegate i lobby member data update listener_1` 相关逻辑。
...
**用途 / Purpose:** 当 `lobby member data update success` 事件触发时调用此方法。
...
**用途 / Purpose:** 当 `lobby member data update failure` 事件触发时调用此方法。
