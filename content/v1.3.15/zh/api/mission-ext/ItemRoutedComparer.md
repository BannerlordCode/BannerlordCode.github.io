...
---
<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `ItemRoutedComparer`
- [← 本领域 / 返回 mission-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.3.15](../../)
...
**类型:** `public class ItemRoutedComparer : SPScoreboardSortControllerVM.ScoreboardUnitItemComparerBase`
**Base:** `SPScoreboardSortControllerVM.ScoreboardUnitItemComparerBase`
**领域:** mission-ext

## 概述

`ItemRoutedComparer` 位于 `TaleWorlds.MountAndBlade.ViewModelCollection.Scoreboard`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
```csharp
// 先从游戏状态中拿到一个 ItemRoutedComparer 实例，再调用它的公开方法
