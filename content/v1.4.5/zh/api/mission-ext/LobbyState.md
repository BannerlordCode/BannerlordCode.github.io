...
**用途 / Purpose:** **用途 / Purpose:** 重新计算并更新 has multiplayer privilege 的最新表示。
...
**用途 / Purpose:** **用途 / Purpose:** 重新计算并更新 has crossplay privilege 的最新表示。
...
**用途 / Purpose:** **用途 / Purpose:** 在 client refused to join custom server 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 重新计算并更新 has user generated content privilege 的最新表示。
...
**用途 / Purpose:** **用途 / Purpose:** 尝试获取 login 的值，通常通过 out 参数返回是否成功。
...
**用途 / Purpose:** **用途 / Purpose:** 调用 HostGame 对应的操作。
...
**用途 / Purpose:** **用途 / Purpose:** 显示feedback对应的界面或元素。
...
**用途 / Purpose:** **用途 / Purpose:** 调用 DismissFeedback 对应的操作。
...
**用途 / Purpose:** **用途 / Purpose:** 在 pause 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 resume 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 cancel finding game 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 player data received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 pending rejoin 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 enter battle with party 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 party invitation received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 party join request received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 admin message received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 party invitation invalidated 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 players added to party 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 game client state change 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 为 connection state 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** **用途 / Purpose:** 在 activate home 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 activate custom server 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 activate matchmaking 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 activate profile 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 clan invitation received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 clan invitation answered 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 clan creation successful 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 clan creation failed 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 clan creation started 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 premade game eligibility status received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 premade game list received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 premade game creation cancelled 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 join premade game requested 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 activate armory 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 activate options 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 deactivate options 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 custom game server list received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 matchmaker game over 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 battle server lost 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 removed from matchmaker game 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 removed from custom game 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 join custom game failure response 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 recent player statuses received 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 在 invite to platform session 事件触发时调用此回调。
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 custom actions for server 的结果。
...
**用途 / Purpose:** **用途 / Purpose:** 将for custom server action注册到当前系统，以便后续监听或分发。
...
**用途 / Purpose:** **用途 / Purpose:** 从当前系统中注销for custom server action。
