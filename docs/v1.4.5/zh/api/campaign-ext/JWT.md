...
**命名空间:** Jose
**模块:** Jose
**类型:** `public static class JWT`
**领域:** campaign-ext

## 概述

`JWT` 位于 `Jose`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
**用途 / Purpose:** 处理 `encode bytes` 相关逻辑。
...
**用途 / Purpose:** 处理 `decode bytes` 相关逻辑。
