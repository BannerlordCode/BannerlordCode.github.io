<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `BCRYPT_OAEP_PADDING_INFO`
...
**Module:** Jose.native
**Type:** `public struct BCRYPT_OAEP_PADDING_INFO`
**Area:** campaign-ext

## Overview

`BCRYPT_OAEP_PADDING_INFO` lives in `Jose.native`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a BCRYPT_OAEP_PADDING_INFO instance from game state, then call one of its public methods
var value = new BCRYPT_OAEP_PADDING_INFO();
