...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `SteamGameServerNetworkingUtils`
...
**Module:** Steamworks
**Type:** `public static class SteamGameServerNetworkingUtils`
**Base:** none
**File:** `Bannerlord.Source/bin/Steamworks.NET/Steamworks/SteamGameServerNetworkingUtils.cs`

## Overview

`SteamGameServerNetworkingUtils` lives in `Steamworks` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
```csharp
SteamGameServerNetworkingUtils.AllocateMessage(0);
```
