...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `DefeatTheConspiracyQuestBehaviorTypeDefiner`
...
`DefeatTheConspiracyQuestBehaviorTypeDefiner` lives in `StoryMode.Quests.ThirdPhase`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a DefeatTheConspiracyQuestBehaviorTypeDefiner instance from game state, then call one of its public methods
var value = new DefeatTheConspiracyQuestBehaviorTypeDefiner();
