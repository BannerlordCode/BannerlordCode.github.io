...
**File:** `Bannerlord.Source/bin/TaleWorlds.CampaignSystem/TaleWorlds.CampaignSystem.CharacterDevelopment/DefaultSkillLevelingManager.cs`
...
**用途 / Purpose:** 当 `hideout cleared as ghost` 事件触发时调用此方法。

### OnHideoutMissionEnd
`public void OnHideoutMissionEnd(bool isSucceeded)`

**用途 / Purpose:** 当 `hideout mission end` 事件触发时调用此方法。
