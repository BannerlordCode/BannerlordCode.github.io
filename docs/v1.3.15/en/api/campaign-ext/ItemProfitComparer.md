<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `ItemProfitComparer`
...
`ItemProfitComparer` lives in `TaleWorlds.CampaignSystem.ViewModelCollection.ClanManagement.Categories`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a ItemProfitComparer instance from game state, then call one of its public methods
