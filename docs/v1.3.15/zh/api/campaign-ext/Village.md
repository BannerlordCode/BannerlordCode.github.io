...
Village 代表依附于城镇或城堡定居点的村庄。它根据 `VillageType` 生产商品，积累炉火（人口）与民兵，并通过 `TradeBound` 进行贸易。Village 是存放在 `Settlement` 上的 `SettlementComponent` 子类之一（与 `Town`、`Hideout` 等并列），可通过 `Settlement.Village` 访问。
...
> 状态变更：直接赋值 `VillageState` 会触发对应战役事件，但推荐的做法是使用 `ChangeVillageStateAction`（如 `OnInit` 中使用的 `ChangeVillageStateAction.ApplyBySettingToNormal(settlement)`）。
