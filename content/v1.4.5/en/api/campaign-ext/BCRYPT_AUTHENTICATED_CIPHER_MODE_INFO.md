...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO`
...
**Module:** Jose.native
**Type:** `public struct BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO : IDisposable`
**Base:** `IDisposable`
**Area:** campaign-ext

## Overview

`BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO` lives in `Jose.native`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO instance from game state, then call one of its public methods
var value = new BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO();
