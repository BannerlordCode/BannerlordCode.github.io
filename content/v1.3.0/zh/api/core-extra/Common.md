...
**用途 / Purpose:** 将object as json序列化为可存储或传输的格式。
...
**用途 / Purpose:** 将object as json string序列化为可存储或传输的格式。
...
**用途 / Purpose:** 调用 FromUrlSafeBase64 对应的操作。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的type。
...
**用途 / Purpose:** 调用 MemoryCleanupGC 对应的操作。
...
**用途 / Purpose:** 调用 DynamicInvokeWithLog 对应的操作。
...
**用途 / Purpose:** 调用 InvokeWithLog 对应的操作。
...
**用途 / Purpose:** 调用 TextContainsSpecialCharacters 对应的操作。
...
**用途 / Purpose:** 将外部输入解析为当前系统可识别的ip address。
