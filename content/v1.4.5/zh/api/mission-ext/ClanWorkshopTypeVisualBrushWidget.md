...
description: "ClanWorkshopTypeVisualBrushWidget 的自动生成类参考。"
---
...
`ClanWorkshopTypeVisualBrushWidget` 是一个 Gauntlet UI 控件——在 Gauntlet XML/.prefab 中使用或代码创建的 UI 元素。继承 Widget 可构建自定义控件；实例经控件树访问。

## 心智模型

把 `ClanWorkshopTypeVisualBrushWidget` 当作一个 Widget 型扩展点来理解：先确认谁创建它、谁持有它、谁调用它，再决定是继承、组合还是只读使用。
...
**用途 / Purpose:** **用途 / Purpose:** 调用 ClanWorkshopTypeVisualBrushWidget 对应的操作。
