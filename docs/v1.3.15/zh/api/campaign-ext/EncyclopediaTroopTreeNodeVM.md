<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `EncyclopediaTroopTreeNodeVM`
- [← 本领域 / 返回 campaign-ext](./)
- [↑ API 目录](../)
...
**命名空间:** TaleWorlds.CampaignSystem.ViewModelCollection.Encyclopedia.Items
...
> 本页为自动生成的存根。`EncyclopediaTroopTreeNodeVM` 是 `TaleWorlds.CampaignSystem.ViewModelCollection.Encyclopedia.Items` 命名空间下的一个类 class。
