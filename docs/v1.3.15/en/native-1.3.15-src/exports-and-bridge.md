...
`WotsMainSDLL` is the most visible entry point from managed startup code. In [LibraryApplicationInterface](../native/LibraryApplicationInterface.md), `MBDotNet.WotsMainDotNet` binds to it through `DllImport("TaleWorlds.Native.dll", EntryPoint = "WotsMainSDLL")`.
...
The managed `[EngineClass("ftdnNative_array")]` attributes are covered in [EngineClass and EngineMethod](../native/EngineMethod.md). On the decompiled side, these strings and vftable writes help trace object creation, reference counting, and cross-boundary return values.
...
This type belongs near `rglScript_component` and `rglManaged_script_component` in the scene script-component family. See [Scene](./scene.md).

## Mapping To Managed Native Reference

| Managed page | Native-side anchor |
|--------------|--------------------|
| [NativeObject](../native/NativeObject.md) | `IManaged` reference counting, `EngineClass` type registration, and `ftdn*` native objects |
| [INativeArray](../native/INativeArray.md) | `ftdnNative_array` strings, vftable writes, and `create/get_data_*` methods |
| [INativeString](../native/INativeString.md) | `ftdnNative_string` strings and string helper paths |
| [INativeObjectArray](../native/INativeObjectArray.md) | `ftdnNative_object_array` strings and object-array methods |
| [LibraryApplicationInterface](../native/LibraryApplicationInterface.md) | `pass_managed_library_callback_method_pointers` and `ScriptingInterfaceObjects.SetFunctionPointer` |
| [EngineMethod](../native/EngineMethod.md) | Mapping between `[EngineMethod]` names and native registration/callback tables |
