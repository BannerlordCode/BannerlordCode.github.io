...
**用途 / Purpose:** 判断当前对象是否处于 「position valid for navigation type」 状态或条件。
...
**用途 / Purpose:** 读取并返回当前对象中 「closest nav mesh face center position for position」 的结果。
...
**用途 / Purpose:** 读取并返回当前对象中 「embark disembark data for tick」 的结果。
...
**用途 / Purpose:** 读取并返回当前对象中 「embark and disembark data for player」 的结果。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的「point around position」。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的「reachable point around position」。
...
**用途 / Purpose:** 在当前集合/范围内查找满足条件的「point inside area」。
...
**用途 / Purpose:** 判断当前对象是否处于 「point inside borders」 状态或条件。
...
**用途 / Purpose:** 读取并返回当前对象中 「interaction data for main party」 的结果。

```csharp
// 静态调用，不需要实例
NavigationHelper.GetInteractionDataForMainParty(settlement, canNavigate, bestNavigationType, isTargetingPort);
```
