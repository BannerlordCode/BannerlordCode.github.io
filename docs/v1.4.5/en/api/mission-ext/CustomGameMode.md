<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `CustomGameMode`
...
`CustomGameMode` lives in `TaleWorlds.MountAndBlade.Multiplayer.ViewModelCollection.Lobby.CustomGame`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
CustomGameMode example = CustomGameMode.CustomServer;
```
