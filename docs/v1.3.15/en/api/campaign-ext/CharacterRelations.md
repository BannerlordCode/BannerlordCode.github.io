<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `CharacterRelations`
- [← Area / Back to campaign-ext](./)
- [↑ API Index](../)
- [🏠 Home v1.3.15](../../)
- [⭐ SDK Overview](../../architecture/sdk-overview)
<!-- END BREADCRUMB -->
# CharacterRelations

**Namespace:** TaleWorlds.CampaignSystem.Settlements.Locations
**Module:** TaleWorlds.CampaignSystem
**Type:** `public enum CharacterRelations`
**Area:** campaign-ext

## Overview

`CharacterRelations` lives in `TaleWorlds.CampaignSystem.Settlements.Locations`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.

## Mental Model

Start from namespace `TaleWorlds.CampaignSystem.Settlements.Locations` to place it in the stack, then inspect its public methods: if it mainly exposes Get/Set members, it is likely a state object; if it centers on Create/Apply/Execute verbs, it behaves more like a service or workflow entry point.

## Enum Values

| Name |
|------|
| `Neutral` |
| `Friendly` |

## Usage Example

```csharp
CharacterRelations example = CharacterRelations.Neutral;
```

## See Also

- [Complete Class Catalog](../catalog)
- [Area catalog](../catalog-campaign)
