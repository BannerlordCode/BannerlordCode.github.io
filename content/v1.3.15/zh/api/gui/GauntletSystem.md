---
title: "GauntletSystem"
---
<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `GauntletSystem / Gauntlet系统事件管理`
- [← 本领域 / 返回 gui](./)
- [↑ API 目录](../)
- [🏠 首页 v1.3.15](../../)
...
先把 `GauntletSystem` 当作这个子系统的入口或数据节点来理解：先看属性代表什么状态，再看方法允许你做什么。

**Namespace**: TaleWorlds.GauntletUI
**File**: 

`TaleWorlds.GauntletUI/EventManager.cs
...
\| 属?\| 类型 \| 描述 \|
\|------\|------\|------\|
\| UIEventManager \| EventManager \| 静态单例实?\|
...
`void BeginDragging(Widget draggedObject)

` \| 开始拖拽widget \|
\| ReleaseDraggedWidget \| 

`Widget ReleaseDraggedWidget()
...
`bool IsPointInsideUsableArea(Vector2 p)
...
`void Render(TwoDimensionContext twoDimensionContext)
...
\| OnDragEnded \| 结束拖拽时触?\|
\| OnFocusedWidgetChanged \| 焦点widget变化时触?\|
