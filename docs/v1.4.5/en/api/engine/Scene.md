...
**Base:** `NativeObject`
**File:** `Bannerlord.Source/bin/TaleWorlds.Engine/TaleWorlds.Engine/Scene.cs`
...
**Purpose:** Sets the value or state of `blocker direction for faces with id`.
...
**Purpose:** Handles the `current frame tick entities` event or callback.
...
**Purpose:** Sets the value or state of `use advanced water rendering`.
