...
---
<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO`
- [← 本领域 / 返回 campaign-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.4.5](../../)
- [⭐ 版本架构](../../architecture/)
...
**命名空间:** Jose.native
**模块:** Jose.native
**类型:** `public struct BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO : IDisposable`
**Base:** `IDisposable`
**领域:** campaign-ext

## 概述

`BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO` 位于 `Jose.native`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
```csharp
// 先从游戏状态中拿到一个 BCRYPT_AUTHENTICATED_CIPHER_MODE_INFO 实例，再调用它的公开方法
