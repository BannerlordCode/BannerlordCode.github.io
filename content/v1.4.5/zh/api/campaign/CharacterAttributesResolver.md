...
description: "CharacterAttributesResolver 的自动生成类参考。"
---
...
**File:** `bin/TaleWorlds.CampaignSystem/TaleWorlds.CampaignSystem.SaveCompability/CharacterAttributesResolver.cs`

## 概述

`CharacterAttributesResolver` 位于 `TaleWorlds.CampaignSystem.SaveCompability`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。
...
```csharp
// 先通过子系统 API 拿到 CharacterAttributesResolver 实例
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 field member with id 的结果。
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 new type 的结果。
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 property member with id 的结果。
