...
`MBTypeMismatchException` 是一个异常类型（继承自 Exception），在特定错误条件发生时抛出。捕获它以处理该错误。

## 使用示例

```csharp
// MBTypeMismatchException (Exception) 的典型用法
try { /* call */ } catch (MBTypeMismatchException e) { /* handle */ };
```
