...
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `DiscriminatedUnionConverter`
...
**Type:** `public class DiscriminatedUnionConverter : JsonConverter`
**Base:** `JsonConverter`
**File:** `Bannerlord.Source/bin/Newtonsoft.Json/Newtonsoft.Json.Converters/DiscriminatedUnionConverter.cs`

## Overview

`DiscriminatedUnionConverter` lives in `Newtonsoft.Json.Converters` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
