...
**文件:** `bannerlord-1.3.15/TaleWorlds.CampaignSystem/Issues/IssueBase.cs`

`IssueBase` 是所有"要人问题"（issue）的基类。要人在城镇/村庄里向玩家提出问题，玩家可选择：完成对应任务（quest）、派遣部队的替代方案（alternative solution），或交由领主处理（lord solution）。mod 通过继承它来添加自定义问题。

## 概述

每个问题挂在一个 `IssueOwner`（提供问题的英雄）上。子类必须实现对话文本属性（`IssueBriefByIssueGiver` 等）与 `IsThereAlternativeSolution` / `IsThereLordSolution` 布尔。替代方案涉及所需兵力（`AlternativeSolutionBaseNeededMenCount`）与时长（`GetBaseAlternativeSolutionDurationInDays`），并可通过 `AlternativeSolutionCondition` / `DoTroopsSatisfyAlternativeSolution` 校验。
...
| IssueQuestSolutionExplanationByIssueGiver | TextObject | 任务解法的说明 |
| IssueQuestSolutionAcceptByPlayer | TextObject | 玩家接受任务解法的台词 |
| IsThereAlternativeSolution | bool | 是否有替代（派兵）解法 |
| IsThereLordSolution | bool | 是否有领主解法 |

## 关键虚成员

| 名称 | 类型 | 描述 |
|------|------|------|
| IssuePlayerResponseAfterLordExplanation | TextObject | 领主解法说明后玩家回应 |
| IssueAlternativeSolutionExplanationByIssueGiver | TextObject | 替代解法说明 |
| AlternativeSolutionBaseNeededMenCount | int | 替代解法所需基础兵力 |
| AlternativeSolutionHero | Hero | 执行替代解法的英雄 |
| CounterOfferHero | Hero | 领主反建议的英雄 |
| NeededInfluenceForLordSolution | int | 领主解法所需影响力 |
| IssueAsRumorInSettlement | TextObject | 作为传闻出现的文本 |
...
{
    public override TextObject IssueBriefByIssueGiver => new TextObject("{=myissue_brief}我的商队被抢了...");
...
::: tip 提示
问题与任务配合：`IssueBase` 描述问题与解法对话，`GetIssueQuest` 返回玩家选择任务解法时启动的 `QuestBase`。详见源文件中的完整虚方法表。
:::
