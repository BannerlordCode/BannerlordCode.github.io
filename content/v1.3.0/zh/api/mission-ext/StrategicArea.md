...
**用途 / Purpose:** 读取并返回当前对象中 「ground position」 的结果。
...
**用途 / Purpose:** 根据当前状态判定「associated destructible components」的结果。
...
**用途 / Purpose:** 在 「parent game entity visibility changed」 事件触发时调用此回调。
...
**用途 / Purpose:** 为 「autogenerated」 初始化必要的资源、状态或绑定。
...
**用途 / Purpose:** 判断当前对象是否处于 「agent eligible」 状态或条件。
...
**用途 / Purpose:** 判断当前对象是否处于 「usable by」 状态或条件。
