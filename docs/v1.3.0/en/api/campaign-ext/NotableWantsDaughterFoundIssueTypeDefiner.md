<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `NotableWantsDaughterFoundIssueTypeDefiner`
...
`NotableWantsDaughterFoundIssueTypeDefiner` lives in `SandBox.Issues`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.
...
```csharp
// First obtain a NotableWantsDaughterFoundIssueTypeDefiner instance from game state, then call one of its public methods
