...
**类型:** `public class ProductInputOutputEqualityComparer : IEqualityComparer<ValueTuple<ItemCategory, int>>`
**Base:** `IEqualityComparer<ValueTuple<ItemCategory`
**领域:** campaign-ext

## 概述

`ProductInputOutputEqualityComparer` 位于 `TaleWorlds.CampaignSystem.ViewModelCollection`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
