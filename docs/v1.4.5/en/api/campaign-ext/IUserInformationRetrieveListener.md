<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `IUserInformationRetrieveListener`
...
`IUserInformationRetrieveListener` lives in `Galaxy.Api` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
**Purpose:** Handles logic related to `swig delegate i user information retrieve listener_0`.
...
**Purpose:** Handles logic related to `swig delegate i user information retrieve listener_1`.
