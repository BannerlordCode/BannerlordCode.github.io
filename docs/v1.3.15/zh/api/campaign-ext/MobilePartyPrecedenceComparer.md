<!-- BEGIN BREADCRUMB -->
**首页** → **API 目录** → **本领域** → `MobilePartyPrecedenceComparer`
- [← 本领域 / 返回 campaign-ext](./)
- [↑ API 目录](../)
- [🏠 首页 v1.3.15](../../)
...
**类型:** `public class MobilePartyPrecedenceComparer : IComparer<MobileParty>`
**Base:** `IComparer<MobileParty>`
**领域:** campaign-ext

## 概述

`MobilePartyPrecedenceComparer` 位于 `TaleWorlds.CampaignSystem.ViewModelCollection`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
