...
它是 vanilla AI 购粮策略：城镇补粮目标为 `30` 天、村庄为 `12` 天、低价基准为 `30`，并从据点库存中按价格和物品价值加权挑选食物或活牲畜。它只输出目标和候选物品，实际缺口由购粮 Behavior 计算，库存与金币由 `SellItemsAction` 执行；所以替换它会影响使用活动模型的 AI 队伍，但不会直接写入交易状态。
