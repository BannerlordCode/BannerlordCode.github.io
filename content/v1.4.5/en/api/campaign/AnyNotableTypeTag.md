...
description: "Auto-generated class reference for AnyNotableTypeTag."
---
# AnyNotableTypeTag
...
`AnyNotableTypeTag` lives in `TaleWorlds.CampaignSystem.Conversation.Tags` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
...
```csharp
// Obtain an instance of AnyNotableTypeTag from the subsystem API first
AnyNotableTypeTag anyNotableTypeTag = ...;
var result = anyNotableTypeTag.IsApplicableTo(character);
```
...
AnyNotableTypeTag anyNotableTypeTag = ...;
anyNotableTypeTag.IsApplicableTo(character);
```
