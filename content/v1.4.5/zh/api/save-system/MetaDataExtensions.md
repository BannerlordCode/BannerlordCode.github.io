...
`MetaDataExtensions` 位于 `TaleWorlds.SaveSystem`，它通过这组公开成员把对应子系统的状态、行为或流程入口暴露给 mod 开发者。阅读时先看属性代表“它持有什么状态”，再看方法代表“它允许你做什么”。
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 application version 的结果。
...
**用途 / Purpose:** **用途 / Purpose:** 读取并返回当前对象中 new game version 的结果。
