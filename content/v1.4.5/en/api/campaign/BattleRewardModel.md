...
**File:** `bin/TaleWorlds.CampaignSystem/TaleWorlds.CampaignSystem.ComponentInterfaces/BattleRewardModel.cs`
...
// Obtain an instance of BattleRewardModel from the subsystem API first
BattleRewardModel battleRewardModel = ...;
var result = battleRewardModel.CalculateRenownGain(winnerParty, 0, 0, 0, false);
```
...
// Obtain an instance of BattleRewardModel from the subsystem API first
BattleRewardModel battleRewardModel = ...;
var result = battleRewardModel.CalculateInfluenceGain(winnerParty, 0, 0, 0, false);
```
...
// Obtain an instance of BattleRewardModel from the subsystem API first
BattleRewardModel battleRewardModel = ...;
var result = battleRewardModel.CalculateMoraleGainVictory(winnerParty, 0, 0, false);
```
...
**Purpose:** Reads and returns the `capture member chances for winner parties` value held by the current object.

```csharp
// Obtain an instance of BattleRewardModel from the subsystem API first
BattleRewardModel battleRewardModel = ...;
battleRewardModel.GetCaptureMemberChancesForWinnerParties(endedMapEvent, winnerParties, mBList<KeyValuePair<MapEventParty, 0, mBList<KeyValuePair<MapEventParty, 0);
```
...
**Purpose:** Reads and returns the `winner parties that can plunder gold from ships` value held by the current object.
...
**Purpose:** Checks whether the current object meets the preconditions for `troop be taken prisoner`.

```csharp
// Obtain an instance of BattleRewardModel from the subsystem API first
BattleRewardModel battleRewardModel = ...;
var result = battleRewardModel.CanTroopBeTakenPrisoner(troop);
```
