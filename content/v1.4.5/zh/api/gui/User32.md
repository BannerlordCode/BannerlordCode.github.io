...
**用途 / Purpose:** 调用 MonitorEnumDelegate 对应的操作。
...
**用途 / Purpose:** 读取并返回当前对象中 async key state 的结果。
...
**用途 / Purpose:** 调用 DestroyWindow 对应的操作。
...
**用途 / Purpose:** 调用 ScreenToClient 对应的操作。
...
**用途 / Purpose:** 读取并返回当前对象中 cursor pos 的结果。
...
**用途 / Purpose:** 调用 ReleaseCapture 对应的操作。
...
**用途 / Purpose:** 为 capture 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 为 active window 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 为 foreground window 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 构建一个新的 window ex 实体并返回给调用方。
...
**用途 / Purpose:** 判断当前对象是否处于 iconic 状态或条件。
...
**用途 / Purpose:** 调用 PeekMessage 对应的操作。
...
**用途 / Purpose:** 调用 TranslateMessage 对应的操作。
...
**用途 / Purpose:** 调用 DispatchMessage 对应的操作。
...
**用途 / Purpose:** 将class注册到当前系统，以便后续监听或分发。
...
**用途 / Purpose:** 调用 DefWindowProc 对应的操作。
...
**用途 / Purpose:** 从持久化存储或流中读取 cursor from file。
...
**用途 / Purpose:** 读取并返回当前对象中 desktop window 的结果。
...
**用途 / Purpose:** 读取并返回当前对象中 client rect 的结果。
...
**用途 / Purpose:** 读取并返回当前对象中 window rect 的结果。
...
**用途 / Purpose:** 为 window pos 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 重新计算并更新 window 的最新表示。
...
**用途 / Purpose:** 为 window long 赋新值，并同步更新对象内部状态。
...
**用途 / Purpose:** 重新计算并更新 layered window 的最新表示。
...
**用途 / Purpose:** 调用 SendMessage 对应的操作。
...
**用途 / Purpose:** 调用 MessageBox 对应的操作。
...
**用途 / Purpose:** 调用 EnumDisplayMonitors 对应的操作。
...
**用途 / Purpose:** 读取并返回当前对象中 monitor info 的结果。
...
User32 user32 = ...;
user32.MonitorEnumDelegate(hMonitor, hdcMonitor, lprcMonitor, lParam);
```
