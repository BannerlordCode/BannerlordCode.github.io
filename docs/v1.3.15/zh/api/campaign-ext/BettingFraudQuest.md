...
**类型:** `public class BettingFraudQuest : QuestBase`
**Base:** `QuestBase`
**领域:** campaign-ext

## 概述

`BettingFraudQuest` 位于 `TaleWorlds.CampaignSystem.Issues`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
...
```csharp
// 先从游戏状态中拿到一个 BettingFraudQuest 实例，再调用它的公开方法
