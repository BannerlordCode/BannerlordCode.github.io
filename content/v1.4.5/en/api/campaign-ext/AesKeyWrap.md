---
title: "AesKeyWrap"
---
<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `AesKeyWrap`
- [← Area / Back to campaign-ext](./)
- [↑ API Index](../)
- [🏠 Home v1.4.5](../../)
- [⭐ Version Architecture](../../architecture/)
<!-- END BREADCRUMB -->
# AesKeyWrap

**Namespace:** Jose
**Module:** Jose
**Type:** `public static class AesKeyWrap`
**Area:** campaign-ext

## Overview

`AesKeyWrap` lives in `Jose`, and its public surface shows that it acts as a formal extension or data entry point for this subsystem.

## Mental Model

Start from namespace `Jose` to place it in the stack, then inspect its public methods: if it mainly exposes Get/Set members, it is likely a state object; if it centers on Create/Apply/Execute verbs, it behaves more like a service or workflow entry point.

## Key Methods

### Wrap
`public static byte Wrap(byte cek, byte kek)`

**Purpose:** Handles logic related to `wrap`.

### Unwrap
`public static byte Unwrap(byte encryptedCek, byte kek)`

**Purpose:** Handles logic related to `unwrap`.

## Usage Example

```csharp
// Prepare the required context, then call the static entry point directly
AesKeyWrap.Wrap(0, 0);
```

## See Also

- [Complete Class Catalog](../catalog)
- [Area catalog](../catalog-campaign)
