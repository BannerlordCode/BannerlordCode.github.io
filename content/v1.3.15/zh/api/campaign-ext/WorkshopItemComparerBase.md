...
**类型:** `public abstract class WorkshopItemComparerBase : IComparer<ClanFinanceWorkshopItemVM>`
**Base:** `IComparer<ClanFinanceWorkshopItemVM>`
**领域:** campaign-ext

## 概述

`WorkshopItemComparerBase` 位于 `TaleWorlds.CampaignSystem.ViewModelCollection.ClanManagement.Categories`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
