<!-- BEGIN BREADCRUMB -->
**Home** → **API Index** → **Area** → `DataSetConverter`
...
**Type:** `public class DataSetConverter : JsonConverter`
**Base:** `JsonConverter`
**File:** `Bannerlord.Source/bin/Newtonsoft.Json/Newtonsoft.Json.Converters/DataSetConverter.cs`

## Overview

`DataSetConverter` lives in `Newtonsoft.Json.Converters` and exposes the state, behavior, or workflow entry points of that subsystem to mod developers through its public members. Read its properties as “what state it owns” and its methods as “what actions it allows”.
