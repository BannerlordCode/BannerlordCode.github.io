...
**类型:** `public abstract class ItemComparerBase : IComparer<ArmyManagementItemVM>`
**Base:** `IComparer<ArmyManagementItemVM>`
**领域:** campaign-ext

## 概述

`ItemComparerBase` 位于 `TaleWorlds.CampaignSystem.ViewModelCollection.ArmyManagement`，它的公开成员表明它是这一子系统暴露给 mod 的一个正式扩展或数据入口。
