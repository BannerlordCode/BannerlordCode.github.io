...
`SettlementNameplateManagerWidget` 是一个 Gauntlet UI 控件——在 Gauntlet XML/.prefab 中使用或代码创建的 UI 元素。继承 Widget 可构建自定义控件；实例经控件树访问。
...
**用途 / Purpose:** **用途 / Purpose:** 为 tlement nameplate manager widget 赋新值，并同步更新对象内部状态。
